...
import static uniresolver.driver.did.hpass.constants.RegistryKeys.*;
import static uniresolver.driver.did.hpass.constants.ResolverKeys.*;
import static uniresolver.result.ResolveResult.build;

public class DidHpassDriver implements Driver {
    private PropertyUtils propertyUtils;
    private JSONUtils jsonUtils;
    private AppIdClient appIdClient;
    private RestClientLoadBalancer loadBalancerForNetwork;
    private RestClientLoadBalancer loadBalancerForRegistry;

    private static Logger log = LoggerFactory.getLogger(DidHpassDriver.class);
    ...
    private static final Pattern DID_DATE_TIME_PATTERN = Pattern.compile("^\\d{4}\\-(0[1-9]|1[012])\\-(0[1-9]|[12][0-9]|3[01])T([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]Z$");
    ...
    public DidHpassDriver(Map<String, Object> properties) {
        ...
        this.appIdClient = new AppIdClient(this.properties, this.httpClient, this.objectMapper, this.jsonUtils, this.propertyUtils, this.messageUtils);
    ...
    private ServerEnvironment retrieveNetworkServers(String identifier) throws ResolutionException {
        ...
        if (!this.isNetworksRegistryEnabled()) {
            ...
            ServerEnvironment serverEnvironment = new ServerEnvironment(this.loadBalancerForNetwork.getAllServers(), REGISTRY_METHOD_GET, staticServerList);
        ...
        try {
            ...
            if (httpResponse.statusCode() / 100 != 2) {
                String message = this.messageUtils.formatMessage("COULD_NOT_RETRIEVE_VALID_HTTP_RESPONSE_FROM_BLOCKCHAIN_NETWORK", httpResponse.uri(), httpResponse.statusCode());
                log.error(message);
                throw new ResolutionException(message);
            }
            response = this.jsonUtils.retrieveBodyAsJsonObject(httpResponse);
        } catch (Exception e) {
            String message = this.messageUtils.formatMessage("COULD_NOT_RETRIEVE_SERVERS_FROM_REGISTRY_FOR_IDENTIFIER", identifier, e.getMessage());
            log.error(message);
            throw new ResolutionException(message, e);
        }

        if (response == null || response.size() == 0 || !response.hasNonNull(REGISTRY_PAYLOAD) || !response.get(REGISTRY_PAYLOAD).hasNonNull(REGISTRY_ENVIRONMENTS)) {
    ...
    private JsonNode fetchDidFromBlockchainNetwork(ServerEnvironment serverEnvironment, String identifier) throws ResolutionException {
        ...
        try {
            ...
            if (httpResponse.statusCode() / 100 != 2) {
                String message = this.messageUtils.formatMessage("COULD_NOT_RETRIEVE_VALID_HTTP_RESPONSE_FROM_DID_NETWORK", httpResponse.uri(), httpResponse.statusCode());
                log.error(message);
                throw new ResolutionException(message);
            }
            response = this.jsonUtils.retrieveBodyAsJsonObject(httpResponse);
